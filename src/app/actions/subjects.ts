...
export async function createSubject(data: FormData) {
  ...
  const { data: qData } = await supabase.from("subjects").select("order_index").eq("course_id", courseId).order("order_index", { ascending: false }).limit(1)
...
export async function updateSubject(data: FormData) {
  ...
  const { error } = await supabase.from("subjects").update({ title, order_index: orderIndex }).eq("id", subjectId)
  if (error) throw new Error(error.message)
  revalidatePath("/admin/materias")
  revalidatePath("/admin/provas")
}
